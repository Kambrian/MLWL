...
%     file=['shearprof_rand',num2str(i),'.t0.5.dat'];
%     file=['shearprofall_rand',num2str(i),'.dat'];
     file=['shearprof_boot',num2str(i),'.dat'];
...
datar.smean{mbin}=mean(svars,2);  %weighted average signal
...
%real data
macro.LENSCATID=0;
data=shearprof_rebin('shearprof.dat',nbinvar);
% data=shearprof_rebin('shearprof.t0.5.dat',nbinvar);
% data=shearprof_rebin('shearprofall.dat',nbinvar);
...
h4=plot(data.rvar{l},signlog(datar.sstd{l}),mtyp{l},'markersize',8);hold on;
...
for i=1:4
    s=data.svar{i};
   h1=plot(data.rvar{i},signlog(s),mtyp{i});
   hold on;
   handles(i)=h1(1);
end
plot([1e-2,1e1],[-4.5,-1.5],'c:',[1e-2,1e1],[4.5,1.5],'c:');
for i=1:4
    s=datar.smean{i};
   h2=plot(data.rvar{i},signlog(s),lltyp{i});
   hold on;
   handles(i)=h2(1);
end
set(gca,'xscale','log');
l=legend([h1,h2],'gama','MC av');set(l,'location','south')
xlabel('r/(Mpc/h) comov');ylabel('$signlog(|\Delta\Sigma(r)|)$');    %/(10^{10}M_\odot/h/(Mpc/h)^2)
% print('-depsc','wlSig_photozCmp.eps');
% title('boosted');
%% noise
myfigure;
handles=zeros(4,1);
for i=1:4
    serr=data.svar{i};
  h= loglog(data.rvar{i},data.esvar{i},mtyp{i});
   hold on;
   handles(i)=h(1);
end
% plot([1e-2,1e1],10.^[-4.5,-1.5],'c:',[1e-2,1e1],10.^[4.5,1.5],'c:');
handles2=handles;
for i=1:4
    s=datar.smean{i};
 h= loglog(data.rvar{i},datar.sstd{i},lltyp{i});
   hold on;
   handles2(i)=h(1);
end
legend([handles(1),handles2(1)],'shear noise','bootstrap noise');
set(gca,'xscale','log');
xlabel('r/(Mpc/h) comov');ylabel('$\sigma_{\Delta\Sigma(r)}$');    %/(10^{10}M_\odot/h/(Mpc/h)^2)
% print('-depsc','bootstrap_noise_cmp.eps');
% title('boosted');
%%
myfigure;
handles=zeros(4,1);
...
set(gca,'xscale','log');
xlabel('r/(Mpc/h) comov');ylabel('$\sigma_{bootstrap}/\sigma_{gama}$');    %/(10^{10}M_\odot/h/(Mpc/h)^2)
print('-depsc','bootstrap_noise_rat.eps');
% title('boosted');
...
xlabel('r/(Mpc/h) comov');ylabel('$\Delta\Sigma_{BootStrap}/\Delta\Sigma_{gama}$');    %/(10^{10}M_\odot/h/(Mpc/h)^2)
print('-depsc','bootstrap_sig_rat.eps');
% title('boosted');
