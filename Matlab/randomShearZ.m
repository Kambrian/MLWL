...
%     randdata{i}=gama_rebin(macro,nbinvar);
    ...
    randdata{i}=shearprof_Zrebin(file);
end

datar.nmean=cell(2,1);
datar.nstd=cell(2,1);
datar.smean=cell(2,1);  %average signal; you need 10^6 realizations to reduce smean from 10^3 to 1
datar.sstd=cell(2,1);
datar.esmean=cell(2,1);  %esmean is comparable to smean, means the uncertainty in the final estimation from the 50 measurements
datar.wmean=cell(2,1);
datar.wstd=cell(2,1);
for zbin=1:2
%mean and var in number profile
nvars=zeros(numel(randdata{1}.nvar{zbin}),Nrand);
for i=1:Nrand
nvars(:,i)=randdata{i}.nvar{zbin};
end
datar.nmean{zbin}=mean(nvars,2);
datar.nstd{zbin}=std(nvars,0,2);
%mean and var in shear signal
svars=nvars;wvars=nvars;esvars=nvars;
for i=1:Nrand
    svars(:,i)=randdata{i}.svar{zbin};
    wvars(:,i)=randdata{i}.wvar{zbin};
    esvars(:,i)=randdata{i}.esvar{zbin};
end
datar.smean{zbin}=sum(svars.*wvars,2)./sum(wvars,2);  %weighted average signal
datar.sstd{zbin}=std(svars,0,2);  %scatter among random catalogues,or single measurement error
datar.esmean{zbin}=sqrt(sum((esvars.*wvars).^2,2))./sum(wvars,2);  %weighted average error est, or error on the estimation using 50 catlogues as a whole 
                        ...
                        % yes
datar.wmean{zbin}=mean(wvars,2);
datar.wstd{zbin}=std(wvars,0,2);
end

%real data
macro.LENSCATID=0;
data=shearprof_Zrebin('shearprof300.dat');
...
data.v=cell(2,1);
for i=1:2
    tmp=logspace(log10(rmin),log10(rmax),60+1)';
    data.v{i}=pi*diff(tmp.^2);
end
%%
data.y=cell(2,1);data.ey=cell(2,1);
datar.y=cell(2,1);datar.ey=cell(2,1);datar.eyp=cell(2,1);
data.wy=cell(2,1);
data.ewy=cell(2,1);
datar.wy=cell(2,1);datar.ewy=cell(2,1);datar.ewp=cell(2,1);
for i=1:2
...
% print('-depsc',[dir,'n_prof_zbincmp_rand_300.eps']);
...
% print('-depsc',[dir,'n_prof_cmp_rand.eps']);
% print('-depsc',[dir,'w_prof_cmp_rand_500.eps']);
...
% print('-depsc',[dir,'n_prof_zbinrat_rand_300.eps']);
...
% print('-depsc',[dir,'w_prof_rat_rand_500.eps']);

c=['r','g'];
myfigure;
for i=1:2
    plot(data.rvar{i},(data.wvar{i}./datar.wmean{i})./(data.y{i}./datar.y{i}),c(i));
    hold on;
end
plot([1e-2,100],[1,1],'--');
ylim([0,2]);
