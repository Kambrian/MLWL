%update the catalogue data to G3Cv4_update6, 
%update G3Cv4 with Stellar Mass and stellermass-derived halo mass from G3Cv6
%%
clear;
clear global;
...
ncol=numel(names);
hdf5write('groupcatv4up6.hdf5','/info','G3Cv4 groups for 3 sky patches, updated StellarMass and HODMasses from G3Cv6');
for sky=1:3
for i=1:ncol
hdf5write('groupcatv4up6.hdf5',['/sky',num2str(sky-1),'/',names{i}],gr{sky}.(names{i}),'writemode','append')
end
end
%%
...
ncol=numel(names);
hdf5write('GAMAgalv4up6.hdf5','/info','gama galaxies G3Cv4 for 3 sky patches, updated StellarMass from G3Cv6');
for sky=1:3
for i=1:ncol
hdf5write('GAMAgalv4up6.hdf5',['/sky',num2str(sky-1),'/',names{i}],g{sky}.(names{i}),'writemode','append')
end
end