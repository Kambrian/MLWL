...
%     file=['shearprofz_rand',num2str(i),'.t0.5.dat'];
    file=['shearprofz_rand',num2str(i),'.dat'];
...
data=shearprof_rebin('shearprofz.dat',nbinvar);
...
legend([handles(1),handles2(1)],'shear noise','photoz noise');
set(gca,'xscale','log');
xlabel('r/(Mpc/h) comov');ylabel('$\sigma_{\Delta\Sigma(r)}$');    %/(10^{10}M_\odot/h/(Mpc/h)^2)
print('-depsc','photoz_noise_cmp.eps');
% title('boosted');
%%
myfigure;
handles=zeros(4,1);
...
set(gca,'xscale','log');
xlabel('r/(Mpc/h) comov');ylabel('$\sigma_{z}/\sigma_{shear}$');    %/(10^{10}M_\odot/h/(Mpc/h)^2)
print('-depsc','photoz_noise_rat.eps');
% title('boosted');
...
for i=1:4
    serr=data.svar{i};
  h= plot(data.rvar{i},data.svar{i}'./datar.smean{i},mtyp{i});
   hold on;
   handles(i)=h(1);
end
set(gca,'xscale','log');hold on;plot(get(gca,'xlim'),[1,1]);
xlabel('r/(Mpc/h) comov');ylabel('$\Delta\Sigma_{gama}/\Delta\Sigma_{MC}$');    %/(10^{10}M_\odot/h/(Mpc/h)^2)
% print('-depsc','photoz_sig_rat.eps');
% title('boosted');
