%% Keeping the bias fixed to theoretical values does not solve the problem of low WL mass
%%
% clear;
% clear global;
load_G3Cv4;
...
%%
[cf1,gof1,out1]=createFit_autobias(r1,s1,es1.^-2,Zbin(1));
ci1=confint(cf1,0.683);
par1=coeffvalues(cf1);
[cf2,gof2,out2]=createFit_autobias(r2,s2,es2.^-2,Zbin(2));
ci2=confint(cf2,0.683);
par2=coeffvalues(cf2);
[cf3,gof3,out3]=createFit_autobias(r3,s3,es3.^-2,Zbin(3));
...
ec=sqrt([err1(2,2),err2(2,2),err3(2,2)])';
% ec=sqrt([err1(3,3),err2(3,3),err3(3,3)])';
cmc=[err1(1,2),err2(1,2),err3(1,2)]';  %correlation between m and b
cmc=cmc./em./ec;
...
plot(r1,signlog(lensing_rfunc_autobias(r1,par1(1),Zbin(1),par1(2))),'r-'); hold on;
plot(r2,signlog(lensing_rfunc_autobias(r2,par2(1),Zbin(2),par2(2))),'g-');
plot(r3,signlog(lensing_rfunc_autobias(r3,par3(1),Zbin(3),par3(2))),'b-');
%%
myfigure;
...
plot(r1,(lensing_rfunc_autobias(r1,par1(1),Zbin(1),par1(2))),'r-'); hold on;
plot(r2,(lensing_rfunc_autobias(r2,par2(1),Zbin(2),par2(2))),'g-');
plot(r3,(lensing_rfunc_autobias(r3,par3(1),Zbin(3),par3(2))),'b-');
...
ploterr(mfit,cfit,em*1e10,ec,'o','logx','logy')
hold on;
Mc=2e12
cguess=10.14.*(1+Zbin).^-1.01.*(mfit'/Mc).^-0.081
plot(mfit,cguess,'-')
x=logspace(12,14,5);
plot(x,10.14.*(x/Mc).^-0.081,'r');
legend('fit','Duffy with z','Duffy');
%%
myfigure;set(gcf,'defaultlinemarkersize',8);
sig8=0.6;
x=logspace(11,14,5);
